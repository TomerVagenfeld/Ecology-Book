# convert_headers_from_numbering.py
# Usage:
#   python convert_headers_from_numbering.py input.docx [output.docx]
#
# Behavior:
#   - Reads numbering.xml (with lxml) to classify each (numId, ilvl) as BULLET or NUMBERED
#   - Converts ONLY NUMBERED levels to Heading {ilvl+1}
#   - Leaves BULLETS intact
#   - Skips paragraphs that contain drawings/pictures (images, shapes) so they never become headings
#   - Treats caption-like paragraphs (איור/תמונה/טבלה/Fig./Figure/Illustration) as normal text
#   - Does NOT inject numbers into text
#   - Strips any residual numbering (w:numPr) from existing Headings
#   - Strips inline emphasis from headings (bold/italic/char styles) using safe .find() lookups
#
# If output.docx is omitted, the input file will be modified in place.
...
def main():
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python convert_headers_from_numbering.py input.docx [output.docx]")
        sys.exit(1)
    input_docx = sys.argv[1]
    output_docx = sys.argv[2] if len(sys.argv) == 3 else None
    convert_numbered_to_headings_keep_bullets(input_docx, output_docx)
    print("Done.")
